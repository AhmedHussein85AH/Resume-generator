...
const Celebi = () => {
  ...
  const Photo = () =>
    (_.get(data, 'resume["@graph"][1].image.contentUrl', "") !== '' && (
      <div className="relative z-40">
        <img
          className="w-full object-cover object-center"
          src={_.get(data, 'resume["@graph"][1].image.contentUrl', "")}
  ...
  const Subnames = () => (
    <h6 className="text-lg tracking-wider uppercase">{((data.resume['@graph'][1].givenName[1]) ? (" ("+data.resume['@graph'][1].givenName.map(function(elem,index){
              if(index > 0 && elem['@value']){
                let name = elem['@value'];
                let familynameIndex = data.resume['@graph'][1].familyName.findIndex(x=>x['@language']===elem['@language']);
                if(familynameIndex >= 0){
                  if(data.resume['@graph'][1].familyName[familynameIndex] && data.resume['@graph'][1].familyName[familynameIndex]['@value']){
                    name += " "+data.resume['@graph'][1].familyName[familynameIndex]['@value'];
                  }
                }
                return name;
              }else{
  ...
  const Names = () => (
    <h1 className="tracking-wide uppercase font-semibold" style={{ fontSize: '2.75em' }}>
          {(Array.isArray(data.resume['@graph'][1].givenName)) ? (data.resume['@graph'][1].givenName[0]['@value']) : (data.resume['@graph'][1].givenName)} {(Array.isArray(data.resume['@graph'][1].familyName)) ? (data.resume['@graph'][1].familyName[0]['@value']) : (data.resume['@graph'][1].familyName)}
        </h1>
  );
  ...
  const Header = () => (
    <header style={styles.header}>
      <div className="ml-6">
        <Names />
        <Subnames />
        <h6 className="text-lg tracking-wider uppercase">{_.get(data, 'resume["@graph"][1].description', "")}</h6>
      </div>
    </header>
  );
    ...
    data.objective.enable && (
      <div className="mb-6">
        <Heading title={data.objective.heading} />
          {_.get(data, 'resume["@graph"][1].seeks',[]).map((x, index) => (
  ...
  const Address = () => (
    (
      data.resume["@graph"][1].address && data.resume["@graph"][1].address.length>0 &&
      data.address.enable && (
        <div className="mb-6">
          {data.resume["@graph"][1].address.filter(x => (Date.parse(x.hoursAvailable.validThrough) - Date.parse(new Date()))>0).map(AddressItem)}
        </div>
      )
    ) || ("")
  );
  ...
  const Contact = () => (
    data.contacts.enable && (
      <div className="mb-6">
        ...
        <Address />
        <ContactItem label="Phone" value={_.get(_.find(data.resume["@graph"][1].contactPoint,{contactType:"Preferred"}), 'telephone', "")} />
        <ContactItem label="Email Address" value={_.get(_.find(data.resume["@graph"][1].contactPoint,{contactType:"Preferred"}), 'email', "")} />
        <ContactItem label="Website" value={_.get(data,'resume["@graph"][1].sameAs[0]',"")} />
      </div>
    )
  );
  ...
  const Work = () =>
    _.get(data, "resume['@graph'][1].hasOccupation", []).length && data.work.enable && (
      <div className="mb-6">
        <Heading title={data.work.heading} />
        {_.get(data, "resume['@graph'][1].hasOccupation", []).filter(x => !_.get(x, '@id', '').endsWith("disable")).map(WorkItem)}
      </div>
    );
  ...
  const Education = () =>
    (_.get(data, "resume['@graph'][1].hasCredential", []).length>0) &&
    data.education.enable && (
      <div className="mb-6">
        <Heading title={data.education.heading} />
        {_.get(data, "resume['@graph'][1].hasCredential", []).filter(x => (!_.get(x, '@id', '').endsWith("disable") && _.get(x, 'credentialCategory', '')==="degree")).map(EducationItem)}
      </div>
    );
  
  const userSkills = () => {
    let workSkills = _.chain(_.get(data, "resume['@graph'][1].hasOccupation", [])).map('skills').flatten();
    
    let awardSkills = _.chain(_.get(data, "resume['@graph'][0].award", [])).map('skill:assesses').flatten();
    
    let educationSkills = _.chain(_.get(data, "resume['@graph'][1].hasCredential", [])).map('teaches').flatten();
    
    let coursesSkills = _.chain(_.get(data, "resume['@graph'][1].hasCredential", [])).map('about').flatten().map('hasCourse').flatten().map('teaches').flatten();
    
    let educationProjectSkills = _.chain(_.get(data, "resume['@graph'][1].hasCredential", [])).map('about').map('workExample').flatten().map('hasPart').flatten().map('teaches').flatten();
    
    let interactionTeachSkills = _.chain(_.get(data, "resume['@graph'][1].interactionStatistic", [])).map('result').flatten().map('teaches').flatten();
    
    let interactionAssessSkills = _.chain(_.get(data, "resume['@graph'][1].interactionStatistic", [])).map('result').flatten().map('assesses').flatten();
  ...
  const Memberships = () =>
    data.memberships.enable && (
      <div className="mb-6">
        ...
        <ul className="list-none text-sm">
          {_.get(data.resume["@graph"][1], 'memberOf', []).map(x => (
    ...
    data.references.enable && (
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 col-gap-4 row-gap-2">
          {_.get(data.resume["@graph"][1], 'interactionStatistic', []).filter(x => _.get(x, 'disambiguatingDescription', '')=== 'Reference').map(ReferenceItem)}
        </div>
      </div>
    );
  ...
  const Languages = () =>
    data.languages &&
    data.languages.enable && (_.get(data, 'resume["@graph"][1].knowsLanguage',[]).length > 0) && (
      <div className="w-3/4 mx-auto mb-6">
        <Heading title={data.languages.heading} />
        <div>{_.get(data, 'resume["@graph"][1].knowsLanguage', []).filter(x => _.get(x, 'name', '') !== '').map(LanguageItem)}</div>
      </div>
    );
  ...
  const Awards = () =>
    ...
    data.awards.enable && (
      <div className="mb-6">
        ...
        {_.get(data.resume["@graph"][0], 'award', []).filter(x => x["skill:title"]!=="").map(AwardItem)}
      </div>
    );
  ...
  const Certifications = () =>
    ...
    data.certifications.enable && (
      <div className="mb-6">
        ...
        {_.get(data, "resume['@graph'][1].hasCredential", []).filter(x => (!_.get(x, '@id', '').endsWith("disable") && _.get(x, 'credentialCategory', '')!=="Degree")).map(CertificationItem)}
      </div>
    );
  ...
  const Extras = () =>
    ...
    data.extras.enable && (
      <div className="mb-6">
        ...
        {_.get(data.resume["@graph"][1], 'identifier', []).filter(x => x['value'] !== '').map(ExtraItem)}
      </div>
    );
