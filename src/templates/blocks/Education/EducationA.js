...
const EducationA = () => {
  ...
  return (_.get(data, "resume['@graph'][1].hasCredential", []).length>0 &&
    data.education.enable) ? (
    <div>
      <Heading>{data.education.heading}</Heading>
      <div className="grid gap-4">
          {_.get(data, "resume['@graph'][1].hasCredential", []).filter(x => (!_.get(x, '@id', '').endsWith("disable") && _.get(x, 'credentialCategory', '')==="degree")).map((x) => (<EducationItem
