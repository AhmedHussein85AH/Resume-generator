...
const WorkA = () => {
  ...
  return (_.get(data, "resume['@graph'][1].hasOccupation", []).length > 0 && data.work.enable) ? (
    <div>
      <Heading>{data.work.heading}</Heading>
      <div className="grid gap-4">
        {_.get(data, "resume['@graph'][1].hasOccupation", []).filter(x => !_.get(x, '@id', '').endsWith("disable")).map((x) => (
