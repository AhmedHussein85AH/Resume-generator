    ...
    (
      ...
      data.address.enable && (
        <div className={mainclassName}>
          ...
          {data.resume["@graph"][1].address.filter(x => (Date.parse(x.hoursAvailable.validThrough) - Date.parse(new Date()))>0).map((x, index) => (<AddressItem x={x} index={index} subclassName={subclassName} key={index} />))}
        </div>
      )
    ) || ("")
);
