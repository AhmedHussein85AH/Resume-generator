  ...
  return (
    'languages' in data && (
      <>
        ...
        {_.get(data.resume["@graph"][1], 'knowsLanguage', []).map((x, index) => (
          <Item
            item={x}
            key={_.get(x,'@id', 'item')}
            index={index}
            ...
            dispatch={dispatch}
            first={index === 0}
            last={index === _.size(_.get(data.resume["@graph"][1], 'knowsLanguage', [])) - 1}
          />
        ))}
...
const AddItem = ({ heading, dispatch }) => {
  ...
  const onSubmit = () => {
    ...
    addItem(dispatch, 'data.resume["@graph"][1].knowsLanguage', item);
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  const [isOpen, setOpen] = useState(false);
  const identifier = `data.resume["@graph"][1].knowsLanguage[${index}].`;
    ...
    <div className="my-4 border border-gray-200 rounded p-5">
      ...
      <div className={`mt-6 ${isOpen ? 'block' : 'hidden'}`}>
        ...
        <ItemActions
          ...
          last={last}
          onChange={onChange}
          type="data.resume['@graph'][1].knowsLanguage"
        />
      </div>
    </div>
  );
