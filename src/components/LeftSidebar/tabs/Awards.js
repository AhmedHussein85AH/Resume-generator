...
const AwardsTab = ({ data, onChange }) => {
  ...
  return (
    <>
      ...
      {_.get(data.resume["@graph"][0], 'award', []).map((x, index) => (
        <Item
          item={x}
          key={_.get(x, '@id', 'main')}
          index={index}
          ...
          dispatch={dispatch}
          first={index === 0}
          last={index === _.size(_.get(data.resume["@graph"][0], 'award', [])) - 1}
        />
      ))}
...
const AddItem = ({ heading, dispatch }) => {
  ...
  const onSubmit = () => {
    ...
    addItem(dispatch, 'data.resume["@graph"][0]["award"]', item);
    addItem(dispatch, 'data.resume["@graph"][1]["award"]', _.get(item, "skill:title", ''));
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  const [isOpen, setOpen] = useState(false);
  const identifier = `data.resume["@graph"][0].award[${index}].`;
  const altidentifier = `data.resume["@graph"][1].award[${index}]`;
    ...
    <div className="my-4 border border-gray-200 rounded p-5">
      ...
      <div className={`mt-6 ${isOpen ? 'block' : 'hidden'}`}>
        ...
        <ItemActions
          ...
          last={last}
          onChange={onChange}
          type="data.resume['@graph'][0].award"
