  ...
  return (
    'references' in data && (
      <>
        ...
        {_.get(data.resume["@graph"][1], 'interactionStatistic', []).filter(x => _.get(x, 'disambiguatingDescription', '')=== 'Reference').map((x, index) => (
          <Item
            item={x}
            key={_.get(x,'@id', 'main')}
            index={index}
            ...
            dispatch={dispatch}
            first={index === 0}
            last={index === _.get(data.resume["@graph"][1], 'interactionStatistic', []).filter(x => _.get(x, 'disambiguatingDescription', '')=== 'Reference').length - 1}
          />
        ))}
...
const AddItem = ({ heading, dispatch }) => {
  ...
  const onSubmit = () => {
    ...
    addItem(dispatch, 'data.resume["@graph"][1].interactionStatistic', item);
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  const [isOpen, setOpen] = useState(false);
  const identifier = `data.resume["@graph"][1].interactionStatistic[${index}].`;
    ...
    <div className="my-4 border border-gray-200 rounded p-5">
      ...
      <div className={`mt-6 ${isOpen ? 'block' : 'hidden'}`}>
        ...
        <ItemActions
          ...
          last={last}
          onChange={onChange}
          type="data.resume['@graph'][1].interactionStatistic"
        />
      </div>
    </div>
  );
