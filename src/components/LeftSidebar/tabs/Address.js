...
const AddressTab = ({ data, onChange }) => {
  ...
  return (
    <>
      ...
      {data.resume["@graph"][1].address && data.resume["@graph"][1].address.map((x, index) => (
        <Item
          dispatch={dispatch}
          first={index === 0}
          index={index}
          item={x}
          key={x["@id"]}
          last={index === data.resume["@graph"][1].address.length - 1}
          onChange={onChange}
        />
      ))}
...
const AddItem = ({ heading, dispatch }) => {
  ...
  const onSubmit = () => {
    ...
    addItem(dispatch, 'data.resume["@graph"][1].address', item);
...
const Item = ({ item, index, onChange, dispatch, first, last }) => {
  const [isOpen, setOpen] = useState(false);
  const identifier = `data.resume["@graph"][1].address[${index}].`;
    ...
    <div className="my-4 border border-gray-200 rounded p-5">
      ...
      <div className={`mt-6 ${isOpen ? 'block' : 'hidden'}`}>
        ...
        <ItemActions
          ...
          last={last}
          onChange={onChange}
          type="data.resume['@graph'][1].address"
